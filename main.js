...
// Добавляем новый код в конец

document.addEventListener("DOMContentLoaded", function () {
    const typeSelect = document.querySelector('.select-container'); // Селектор "Тип учебного заведения"
    const regionSelect = document.querySelector('#region-select'); // Селектор "Регион учебных заведений"

    // Функция для изменения границы
    function highlightBorder(element) {
        element.style.borderColor = '#00B0D9';
    }
    ...
    // Обработчики событий для первого селектора
    typeSelect.addEventListener('focus', function () {
        highlightBorder(typeSelect);
    });

    typeSelect.addEventListener('blur', function () {
        resetBorder(typeSelect);
    });

    // Обработчики событий для второго селектора
    regionSelect.addEventListener('focus', function () {
        highlightBorder(regionSelect);
    });

    regionSelect.addEventListener('blur', function () {
        resetBorder(regionSelect);
    });
});

// Ваш другой код, если есть
