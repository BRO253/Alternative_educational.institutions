class Navbar extends HTMLElement {
  ...
  THIRD_PANEL_TAB_TEMPLATE = (
    ...
    const isHighlighted = tabName === "Альтернативные";
    const highlightedStyle = isHighlighted ? "color: var(--yellow) !important; font-weight: bold;" : "";
  
    // Определяем условие для "вакансии объед"
    const isVakansiiObed = tabName === "вакансии объед";
  
    // Генерируем дополнительный стиль для текста с многоточиями при ширине экрана <= 880px
    const ellipsisStyle = `
    ...
    return /*html*/ `
      <div class="third-panel__tab" ${id ? `id=${id.trim()}` : ""} style="${highlightedStyle}">
          ${
            icon
              ? `<span class="third-panel__tab-icon icon ads-icon in-panel ${classes}" style="${highlightedStyle}">${icon}</span>`
              : ""
          }
          <p class="third-panel__tab-text ${tabNameClasses}" id="${id === 'nine' ? 'nine' : ''}" style="${highlightedStyle} ${
      isVakansiiObed ? ellipsisStyle : ""
    }">${tabName}</p>
      </div>
    `;
  };
    ...
    this.innerHTML = /*html*/ `
        ${STYLE}
            ...
            <div id="navbar">
                ...
                <div class="third-panel journals panels__panel" id="nord_s">
                    ${(() => {
                      let markup = "";
